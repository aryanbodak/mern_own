// ═══════════════════════════════════════════════════════════════════════════
//  CourseDB — MongoDB Setup Commands
//  File: database/mongo_commands.js
//
//  Usage:
//    Option 1 (mongosh):  mongosh < mongo_commands.js
//    Option 2 (Node.js):  node mongo_commands.js
//
//  This script will:
//    1. Connect to / create the "courseDB" database
//    2. Create all collections with validators
//    3. Insert the admin user
//    4. Insert sample courses with full topic/subtopic content
...
async function setup() {
  ...
  // ── Drop existing collections (fresh setup) ──────────────────────────────
  const existingCollections = (await db.listCollections().toArray()).map(c => c.name);
  if (existingCollections.includes("users"))   await db.collection("users").drop();
  if (existingCollections.includes("courses")) await db.collection("courses").drop();
    ...
    validator: {
      $jsonSchema: {
        ...
        properties: {
          ...
          role:            { bsonType: "string",  enum: ["user", "admin"], description: "user or admin" },
        ...
        },
        {
          ...
          subTopics: [
            ...
            { subTopicName: "useEffect",
              data: "useEffect runs side effects after renders. The dependency array controls when it fires. An empty array [] means run once on mount. Return a cleanup function to unsubscribe." },
            { subTopicName: "useContext",
              data: "useContext lets you access context values (like theme or auth) anywhere in the component tree without prop-drilling. Pair with React.createContext and a Provider." }
          ]
        }
      ]
    },
