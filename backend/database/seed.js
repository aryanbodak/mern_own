/**
 * database/seed.js — CourseDB Incremental Seeder (Mongoose)
 * ----------------------------------------------------------
 * Run with:  node database/seed.js
 *
 * Options:
 *   node database/seed.js           → seeds only what's missing
 *   node database/seed.js --reset   → wipes DB and re-seeds everything fresh
 */
...
const DB_URL = process.env.MONGO_URI || "mongodb://127.0.0.1:27017/courseDB";
...
const SAMPLE_COURSES = [
  ...
  /* ================================================================
     EXISTING COURSES — expanded with more subtopics
  ================================================================ */
      ...
      },
      {
        ...
        subTopics: [
          ...
          { subTopicName: "CI/CD Basics",
            data: "CI/CD (Continuous Integration/Continuous Deployment) automates testing and deployment. GitHub Actions can run your test suite on every push and auto-deploy to production if tests pass. A basic workflow file lives in .github/workflows/deploy.yml." }
        ]
      }
    ]
  },
      ...
      },
      {
        ...
        subTopics: [
          ...
          { subTopicName: "Dimensionality Reduction (PCA)",
            data: "PCA (Principal Component Analysis) reduces the number of features while preserving variance. Projects data onto principal components ordered by explained variance. Useful for visualization and removing noise before training models." }
        ]
      },
      {
    ...
    topics: [
      {
        ...
        subTopics: [
          ...
          { subTopicName: "IAM & Security",
            data: "IAM (Identity and Access Management) controls who can do what in AWS. Create users, groups, and roles. Attach policies (JSON documents) that define allowed actions. Always follow least privilege principle — give only the permissions needed." }
        ]
      },
      {
        ...
        subTopics: [
          ...
          { subTopicName: "DynamoDB",
            data: "DynamoDB is AWS's fully managed NoSQL database. Stores data as items in tables. Primary key = partition key + optional sort key. Auto-scales for any traffic level. Pay per read/write capacity unit. Great for serverless and high-scale apps." }
        ]
      }
    ]
  },
      ...
      },
      {
        ...
        subTopics: [
          ...
          { subTopicName: "Google Analytics 4",
            data: "GA4 tracks user behaviour on your site. Key metrics: sessions, bounce rate, conversion rate, average session duration. Set up events and goals. Use Acquisition reports to see which channels drive the most traffic. Connect with Google Search Console for SEO data." }
        ]
      }
    ]
  },
  ...
  /* ================================================================
     NEW COURSES — 7 brand new topics
  ================================================================ */
      ...
      },
      {
        ...
        subTopics: [
          ...
          { subTopicName: "Security Auditing Tools",
            data: "Regularly audit your code and dependencies. npm audit scans for known vulnerabilities in packages. OWASP ZAP is a free tool to scan your web app for common vulnerabilities. Burp Suite is used by professionals for penetration testing. Run Lighthouse in Chrome DevTools for a quick security/performance audit." }
        ]
      }
    ]
  },
    ...
    topics: [
      {
        ...
        subTopics: [
          ...
          { subTopicName: "JOINs",
            data: "JOINs combine rows from multiple tables. INNER JOIN returns matching rows from both tables. LEFT JOIN returns all rows from the left table plus matches from the right (nulls where no match). RIGHT JOIN is the reverse. Example: SELECT u.name, o.total FROM users u INNER JOIN orders o ON u.id = o.user_id." },
      ...
      },
      {
        ...
        subTopics: [
          { subTopicName: "Normalisation",
            data: "Normalisation removes data redundancy. 1NF: atomic values, no repeating groups. 2NF: no partial dependencies (non-key columns depend on the whole key). 3NF: no transitive dependencies. A properly normalised schema is easier to maintain and less prone to anomalies." },
          { subTopicName: "Indexes & Performance",
            data: "Indexes speed up queries at the cost of slower writes and more storage. CREATE INDEX idx_email ON users(email). Use EXPLAIN to see if a query uses an index. Avoid indexing every column — index columns used in WHERE, JOIN, and ORDER BY. Composite indexes for multi-column queries." },
    ...
    topics: [
      {
        ...
        subTopics: [
          ...
          { subTopicName: "Queries & Mutations",
            data: "A Query fetches data: query { user(id: '1') { name email } }. A Mutation modifies data: mutation { createUser(name: 'John') { id name } }. Variables make queries dynamic: query GetUser($id: ID!) { user(id: $id) { name } }. Send variables separately from the query string." }
        ]
      },
      {
        ...
        subTopics: [
          ...
          { subTopicName: "Apollo Server with Node.js",
            data: "Apollo Server is the most popular GraphQL server for Node.js. Install: npm install @apollo/server graphql. Define typeDefs (schema string) and resolvers (object). Pass to ApolloServer and start. Use Apollo Studio (sandbox) to test queries in the browser — like Postman for GraphQL." }
        ]
      }
    ]
  },
    ...
    topics: [
      {
        ...
        subTopics: [
          ...
          { subTopicName: "Navigation with React Navigation",
            data: "React Navigation handles screens in React Native. Install @react-navigation/native. Stack Navigator: screens slide left/right like a browser. Tab Navigator: bottom tab bar. Drawer Navigator: slide-out menu. useNavigation() hook to navigate programmatically. useRoute() to access params." }
        ]
      },
      {
...
async function seed() {
  ...
  // Sample courses — delete existing and re-insert to pick up subtopic changes
  let created = 0;
  let updated = 0;
  for (const c of SAMPLE_COURSES) {
    const exists = await Course.findOne({ title: c.title });
    if (exists) {
      await Course.findOneAndUpdate({ title: c.title }, c);
      updated++;
    } else {
  ...
  if (created > 0) console.log(`📚  ${created} new course(s) inserted`);
  if (updated > 0) console.log(`🔄  ${updated} existing course(s) updated with new subtopics`);
